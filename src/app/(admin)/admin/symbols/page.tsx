...
export default function SymbolsPage() {
  ...
  useEffect(() => {
    void fetchQuotes().then((q) => { setQuotes(q); setLoadingQ(false); });
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return symbols;
    return symbols.filter((s) => s.symbol.includes(q) || s.base_asset.includes(q));
  }, [symbols, deferred]);
  ...
  async function handleWatch(sym: string) {
    ...
    try {
      ...
      setAdding((p) => { const n = new Set(p); n.delete(sym); return n; });
    }
  }

  const visible       = filtered.slice(0, 200);
  const activeCount   = quotes.find((q) => q.quote_asset === activeQuote)?.symbol_count
                        ?? symbols.length;
    ...
    <div className="sb-shell">
      ...
      <div className="sb-hero">
        <div className="sb-hero-left">
          <div className="sb-eyebrow"><AppstoreOutlined /> MARKET EXPLORER</div>
      ...
      {loadingS ? (
        ...
        <div className="sb-empty">
          ...
          <p className="sb-empty-title">No pairs found for &ldquo;{search}&rdquo;</p>
          <p className="sb-empty-hint">Try a different name or switch the quote tab</p>
        </div>
      ) : (
        <div className="sb-list-panel">
              ...
              return (
                <div key={item.symbol} className="sb-item">
                  ...
                  <span
                    ...
                    style={{ color: tclr, background: `${tclr}18`, borderColor: `${tclr}35` }}
                  >
                    {item.quote_asset}
                  </span>
          ...
          {filtered.length > 200 && (
            <div className="sb-overflow-hint">
              <span>Showing <strong>200</strong> of <strong>{filtered.length.toLocaleString()}</strong> pairs</span>
