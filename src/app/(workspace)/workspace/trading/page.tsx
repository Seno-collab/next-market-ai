...
export default function TradingPage() {
  ...
  const [restTrades, setRestTrades] = useState<TradeUpdate[]>([]);

  /* ── REST fallback: keep order book fresh if WS lags ── */
  ...
  useEffect(() => {
    if (ticker) {
      hasWsTickerRef.current = true;
      wsTickerUpdateAtRef.current = Date.now();
    } else {
      hasWsTickerRef.current = false;
    }
  }, [ticker]);

  useEffect(() => {
    if (orderBook) {
      hasWsBookRef.current = true;
      wsBookUpdateAtRef.current = Date.now();
    } else {
      hasWsBookRef.current = false;
    }
  }, [orderBook]);
  ...
  useEffect(() => {
    ...
    const fetchBootstrapTrades = async () => {
      try {
        const items = await tradingApi.getTrades(
          symbol,
          100,
          controller.signal,
        );
        ...
        setRestTrades(
          items.map((t) => ({
            ...
            qty: t.qty,
            time: t.time,
            // REST uses is_buyer_maker (opposite naming vs WS is_buyer).
            is_buyer: !t.is_buyer_maker,
          })),
        );
      } catch {
        if (!disposed) {
  ...
  useEffect(() => {
    ...
    const fetchRestTicker = async () => {
      ...
      try {
        const nextTicker = await tradingApi.getTicker(
          symbol,
          controller.signal,
        );
        if (!disposed) {
    ...
    // Prime immediately on symbol change.
  ...
  useEffect(() => {
    ...
    const fetchRestBook = async () => {
      ...
      activeController = controller;
      try {
        const book = await tradingApi.getOrderBook(
          symbol,
          20,
          controller.signal,
        );
        ...
        setRestBook((prev) => {
          ...
          return nextBook.lastUpdateId >= prev.lastUpdateId ? nextBook : prev;
        });
      } catch {
        /* keep previous REST snapshot */
      } finally {
        if (activeController === controller) {
          activeController = null;
        }
  ...
  const wsTickerAgeMs = Date.now() - wsTickerUpdateAtRef.current;
  const wsTickerFresh =
    ticker !== null &&
  ...
  const displayBook =
    orderBook && restBook
      ? restBook.lastUpdateId > orderBook.lastUpdateId
        ? restBook
        : orderBook
      : (orderBook ?? restBook);
  ...
  useEffect(() => {
    function onResize() {
      ...
      setChartHeight(
        w < 480 ? 220 : w < 640 ? 280 : w < 960 ? 340 : w < 1280 ? 400 : 480,
      );
    }
    onResize();
  ...
  const maxTradeQty =
    displayTrades.length > 0
      ? Math.max(...displayTrades.map((t) => parseFloat(t.qty)))
      : 0;
  const buyCount = displayTrades.filter((t) => t.is_buyer).length;
  const tradeRatioPct =
    displayTrades.length > 0
  ...
  return (
    <div className="tp-shell">
      ...
      >
        ...
        {displayTicker ? (
          <div
            className={`tp-stats-strip${isStale ? " tp-stale" : ""}${!ticker && tickerSnapshot && !restTicker ? " tp-snapshot" : ""}`}
          >
            ...
            <div className="tp-stat-item tp-hide-xs">
              ...
              <span className="tp-stat-value">
                {fmtQty(displayTicker.volume)}
              </span>
            </div>
            <div className="tp-stat-item tp-hide-sm">
              <span className="tp-stat-label">High</span>
              <span className="tp-stat-value tp-up">
                {fmt(displayTicker.high_price)}
              </span>
            </div>
            <div className="tp-stat-item tp-hide-sm">
              <span className="tp-stat-label">Low</span>
              <span className="tp-stat-value tp-dn">
                {fmt(displayTicker.low_price)}
              </span>
            </div>
            <div className="tp-stat-item tp-hide-sm">
              <span className="tp-stat-label">VWAP</span>
              <span className="tp-stat-value">
                {fmt(displayTicker.weighted_avg_price)}
              </span>
            </div>
            <div className="tp-stat-item tp-hide-sm">
              <span className="tp-stat-label">Trades</span>
              <span className="tp-stat-value">
                {displayTicker.trade_count.toLocaleString()}
              </span>
            </div>
        ...
        <div className="tp-right-col">
                  ...
                  <div key={row.price} className="tp-book-row tp-ask-row">
                    ...
                    <span className="tp-book-size">
                      ...
                      {bookChanges.get(row.price) === "up" && (
                        <span
                          key={`u-${row.price}`}
                          className="tp-chg-ind tp-chg-up"
                        >
                          ▲
                        </span>
                      )}
                      {bookChanges.get(row.price) === "down" && (
                        <span
                          key={`d-${row.price}`}
                          className="tp-chg-ind tp-chg-dn"
                        >
                          ▼
                        </span>
                      )}
          ...
          <div className={`tp-trades-card${isStale ? " tp-stale" : ""}`}>
            ...
            <div className="tp-trades-list">
              {displayTrades.slice(0, 20).map((t) => {
